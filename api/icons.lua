...
---Creates a two-layer icon from the given `icon`: an untinted base, and a copy tinted with `tint`
---at 75% opacity layered on top.
---
---#### Parameters
...
---```
---@throws Thrown when `icon` is not a mod-relative file path.
---@throws Thrown when `tint` is not a `Color`.
---@throws Thrown when `icon_size` is not a sprite size in pixels.
...
---```
---@throws Thrown when `symbol` is not one of `"area-drill"`, `"filter"`, `"shield"`, `"aperture-open"`, or `"aperture-closed"`.
---@throws Thrown when `tint` is not a `Color`.
...
---```
---@throws Thrown when `letter` is not one of `"F"`, `"H"`, `"L"`, `"M"`, or `"S"`.
---@throws Thrown when `tint` is not a `Color`.
